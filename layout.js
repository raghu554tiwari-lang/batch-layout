/* ============================================================
   PW-MARCO CLOUDFLARE OVERLAY + VPN DETECTION
   ============================================================ */
(function () {
  if (window.__marcoCfVpn) return;
  window.__marcoCfVpn = true;

  /* --- CSS: Body hide karo, overlay visible --- */
  var st = document.createElement('style');
  st.innerHTML = [
    'body{visibility:hidden!important}',
    '#marco-cf-overlay{visibility:visible!important;position:fixed!important;top:0;left:0;width:100vw;height:100vh;background:#fff;z-index:2147483647;display:flex;flex-direction:column;align-items:center;justify-content:center;font-family:-apple-system,sans-serif}',
    '#marco-cf-overlay *{visibility:visible!important}',
    '#marco-cf-title{font-size:28px;font-weight:900;color:#111;margin:0 0 6px}',
    '#marco-cf-sub{font-size:17px;font-weight:700;color:#222;margin:0 0 10px}',
    '#marco-cf-desc{font-size:13px;color:#555;text-align:center;max-width:300px;line-height:1.5;margin:0 0 24px;padding:0 16px}',
    '#marco-cf-spin{width:38px;height:38px;border:4px solid #eee;border-top:4px solid #43e97b;border-radius:50%;animation:mSpin .8s linear infinite}',
    '#marco-cf-ver{font-size:12px;color:#aaa;margin-top:10px}',
    '@keyframes mSpin{from{transform:rotate(0)}to{transform:rotate(360deg)}}',
    '#marco-vpn-popup{position:fixed!important;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.88);z-index:2147483647;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);font-family:-apple-system,sans-serif}',
  ].join('');
  ...
  /* --- Overlay dikhao --- */
  function showCf() {
    if (document.getElementById('marco-cf-overlay')) return;
    var d = document.createElement('div');
    d.id = 'marco-cf-overlay';
    d.innerHTML =
      '<div id="marco-cf-title">PW-MARCO</div>' +
      '<div id="marco-cf-sub">Performing security verification</div>' +
      '<div id="marco-cf-desc">This website uses a security service to protect against malicious bots. This page is displayed while the website verifies you are not a bot.</div>' +
      '<div id="marco-cf-spin"></div>' +
      '<div id="marco-cf-ver">Verifying...</div>';
    (document.body || document.documentElement).appendChild(d);
  }

  /* --- Normal page pe body restore --- */
  function restoreBody() {
    if (document.getElementById('marco-cf-overlay')) return;
    var s = document.createElement('style');
    s.innerHTML = 'body{visibility:visible!important}';
    (document.head || document.documentElement).appendChild(s);
  }

  function check() { isCf() ? showCf() : restoreBody(); }
  ...
  /* Title fix */
  var tFix = setInterval(function(){
    if (document.title && /pwthor/i.test(document.title))
      document.title = document.title.replace(/pwthor\.live/gi,'PW-MARCO').replace(/pwthor/gi,'PW-MARCO');
  }, 200);
  setTimeout(function(){ clearInterval(tFix); }, 15000);

  /* ---- VPN DETECTION ---- */
  function showVpn() {
    if (document.getElementById('marco-vpn-popup')) return;
    var pop = document.createElement('div');
    pop.id = 'marco-vpn-popup';
    pop.innerHTML =
      '<div style="background:#fff;border-radius:20px;padding:26px 20px;max-width:310px;width:88%;box-shadow:0 20px 60px rgba(0,0,0,.5);text-align:center">' +
        '<div style="font-size:44px;margin-bottom:10px">🔒</div>' +
        '<div style="font-size:16px;font-weight:800;color:#e53e3e;margin-bottom:6px">VPN Detected!</div>' +
        '<div style="font-size:12px;color:#555;margin-bottom:16px;line-height:1.6">PW-MARCO VPN ke saath kaam nahi karta.<br>Neeche diye steps se VPN band karein.</div>' +
        '<div style="background:#fff8e1;border-radius:12px;padding:14px;text-align:left;margin-bottom:16px;border:1px solid #ffe082">' +
          '<div style="font-size:12px;font-weight:700;color:#333;margin-bottom:8px">📋 VPN Band Karne ke Steps:</div>' +
          '<div style="font-size:11px;color:#444;line-height:2.1">' +
            '1️⃣ &nbsp;Phone ki <b>Settings</b> kholen<br>' +
            '2️⃣ &nbsp;<b>VPN</b> ya <b>Network &amp; Internet</b> pe jayen<br>' +
            '3️⃣ &nbsp;Connected VPN ko <b>Disconnect</b> karen<br>' +
            '4️⃣ &nbsp;VPN app se bhi <b>Off</b> karen<br>' +
            '5️⃣ &nbsp;<b>PW-MARCO app reopen</b> karen' +
          '</div>' +
        '</div>' +
        '<div style="background:linear-gradient(135deg,#43e97b,#38f9d7);border-radius:10px;padding:12px;font-size:12px;color:#fff;font-weight:600;margin-bottom:10px">✅ VPN band karne ke baad app reopen<br>karein — popup khud hat jayega</div>' +
        '<div style="font-size:10px;color:#bbb">PW-MARCO • Powered by Marco</div>' +
      '</div>';
    ...
    var ri = setInterval(function(){
      ...
      if (['Asia/Kolkata','Asia/Calcutta'].includes(tz) || tries > 120) {
        clearInterval(ri);
        var p = document.getElementById('marco-vpn-popup');
        if (p) p.remove();
  ...
  function vpnCheck() {
    var tz = '';
    try{ tz = Intl.DateTimeFormat().resolvedOptions().timeZone; }catch(e){}
    if (!['Asia/Kolkata','Asia/Calcutta'].includes(tz)) { showVpn(); }
  }

  if (document.readyState === 'complete') setTimeout(vpnCheck, 2000);
  else window.addEventListener('load', function(){ setTimeout(vpnCheck, 2000); });

})();

/* ============================================================
   YAHAN SE TUMHARA ORIGINAL CODE SHURU HOTA HAI — KUCH MAT CHHEDO
   ============================================================ */
